...
"use strict";Nauper.UI=function(e){this.engine=e,this.canvas=this.engine.canvas,this.render=this.engine.render,this.size=this.engine.size,this.lastActive=void 0},Nauper.UI.prototype.setBackground=function(e){e&&(this.canvas.style.backgroundImage="url(./data/images/backgrounds/"+e+")")},Nauper.UI.prototype.drawTextBox=function(e){var t=this,i={type:"default",color:"#fff",link:"",x:.025,y:.8,height:.18,width:.95,radius:.05,callback:function(){}},r=putDefaults(i,e),n=this.size.width*r.x,h=this.size.height*r.y,s=this.size.height*r.height,a=this.size.width*r.width,d=this.size.height*r.radius;this.render.fillStyle=r.color,"default"===r.type?this.render.fillRect(n,h,a,s):"rounded"===r.type?(this.render.beginPath(),this.render.moveTo(n,h+d),this.render.lineTo(n,h+s-d),this.render.quadraticCurveTo(n,h+s,n+d,h+s),this.render.lineTo(n+a-d,h+s),this.render.quadraticCurveTo(n+a,h+s,n+a,h+s-d),this.render.lineTo(n+a,h+d),this.render.quadraticCurveTo(n+a,h,n+a-d,h),this.render.lineTo(n+d,h),this.render.quadraticCurveTo(n,h,n,h+d),this.render.fill()):"image"===r.type&&!function(){var e=new Image;e.addEventListener("load",function(i){t.render.drawImage(e,n,h,a,s),r.callback&&r.callback(i)}),e.src=r.link}(),"image"!==r.type&&r.callback()},Nauper.UI.prototype.drawText=function(e){var t=this,i={text:"",align:"wrapped",color:"#000",x:.1,y:.85,width:.8},r=putDefaults(i,e),n=this.size.width*r.x,h=this.size.height*r.y,s=this.size.width*r.width;if(this.render.fillStyle=r.color,"wrapped"===r.align)!function(){var e=wrapText(t.render,r.text,t.render.font,s);e.result.forEach(function(i,r){t.render.fillText(i,n,h+e.height*r)})}();else if("center"===r.align){var a=getTextOffset(this.render,{width:this.canvas.width},r.text);this.render.fillText(r.text,a,h)}},Nauper.UI.prototype.process=function(e){var t=void 0;return e&&(t="next"),t},Nauper.UI.prototype.move=function(e){if("choice"===this.engine.element.type){for(var t=e.pageX,i=e.pageY,r=!1,n=0;n<this.engine.element.map.length;n+=1){var h={x:.025*this.size.width,y:(.25*n+.025)*this.size.height,height:.2*this.size.height,width:.95*this.size.width};if(t>=h.x&&t<=h.x+h.width&&i>=h.y&&i<=h.y+h.height){this.engine.element.active=n,r=!0;break}}r||(this.engine.element.active=void 0),this.engine.element.active!==this.lastActive&&(this.lastActive=this.engine.element.active,this.engine.element.draw())}};
...
"use strict";Nauper.Question=function(i,t){this.background=t.background,this.activebox=t.textbox.active,this.inactivebox=t.textbox.inactive,this.boxtype=t.textbox.type,this.boxlink=t.textbox.link,this.type="choice",this.necessary=t.necessary,this.engine=i,this.render=this.engine.render,this.canvas=this.engine.canvas,this.size=this.engine.size,this.map=t.map,this.active=void 0,this.draw=function(){var i=this;if(0!==this.map.length&&this.map.length<=4){var t=.025,e=.2,n=.95,s=.05;this.render.clearRect(0,0,this.size.width,this.size.height),this.engine.ui.setBackground(this.background);for(var a=function(a){var h=i.map[a],c=void 0;c=void 0!==i.active&&i.active===a?i.activebox:i.inactivebox,i.engine.ui.drawTextBox({type:i.boxtype,color:c.background,link:i.boxlink,y:.25*a+.025,x:t,height:e,width:n,radius:s,callback:function(){i.engine.ui.drawText({text:h.text,align:"center",color:c.text,y:.25*a+.125})}})},h=0;h<this.map.length;h+=1)a(h)}else this.engine.nextElement()}.bind(this)};
